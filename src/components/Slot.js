...
export class TimeSlot {
    ...
    validate() {
        const start_hour = this.start.getHours(); const start_minute = this.start.getMinutes();
        const end_hour = this.end.getHours(); const end_minute = this.end.getMinutes();

        if (this.start >= this.end) {
            this.valid = false;
        }

        else if (timeCompare(start_hour, start_minute, 10, 30) < 0 || timeCompare(start_hour, start_minute, 17, 0) > 0) {
            this.valid = false;
        }
...
export function Select({state, setState, index, added, start, setStart, open, setOpen}) {
    ...
    return (
        <DatePicker
            modal
            open={open}
            date={(start && added) ? new Date() : ((start || added) ? new Date(now.getFullYear(), now.getMonth(), now.getDate(), start_.getHours(), start_.getMinutes()) : new Date(now.getFullYear(), now.getMonth(), now.getDate(), end_.getHours(), end_.getMinutes()))}
