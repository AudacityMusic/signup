...
/**
 * Get stored Apple user data for subsequent sign-ins.
 * @returns {Promise<object|null>} Apple user data or null if not found
 */
export async function getAppleUserData() {
  try {
    const appleUserData = await AsyncStorage.getItem("apple-user-data");
    if (appleUserData) {
      return JSON.parse(appleUserData);
    }
    return null;
  } catch (error) {
...
/**
 * Update stored Apple user data (for manual updates by user).
 * @param {object} userData - Object containing name and/or email
 * @returns {Promise<boolean>} Success status
 */
export async function updateAppleUserData(userData) {
  try {
    const existingData = await getAppleUserData();
    const updatedData = {
      ...existingData,
      ...userData,
    ...
    await AsyncStorage.setItem("apple-user-data", JSON.stringify(updatedData));

    // Also update the current user session data
    ...
    console.error("Error updating Apple user data:", error);
    return false;
  }
}

/**
 * Clear stored Apple user data (for privacy compliance).
 * @returns {Promise<boolean>} Success status
 */
export async function clearAppleUserData() {
  try {
    await AsyncStorage.removeItem("apple-user-data");
    return true;
  } catch (error) {
